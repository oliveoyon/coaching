...
class UpdateStudentEnrollmentRequest extends FormRequest
{
    ...
    {
        return $this->route('enrollment') !== null
            && ($this->user()?->can('update', $this->route('enrollment')) ?? false);
    }
    ...
    {
        /** @var \App\Models\StudentEnrollment $enrollment */
            ...
            function ($validator): void {
                ...
                if ($user->isTeacher() && $user->teacher) {
                    if ($student->owner_teacher_id !== $user->teacher->getKey()) {
                        $validator->errors()->add('student_id', 'You can only manage enrollments for students within your own scope.');
                    }

                    if ($batch->owner_teacher_id !== $user->teacher->getKey()) {
                        $validator->errors()->add('batch_id', 'You can only manage enrollments for your own batches.');
                    }
                }
            },
        ];
